...
local Owner = "chhinhlongdev"
local Repository = "SenSokScript"
        ...
        local function RunQueue(Options)
            ...
            if not Success then
                ...
                task.delay(3, function()
                    if _ENV.error_message then
                        _ENV.error_message.Text = "SenSokScript Shield\nStart Refresh Options ..."
...
AddModule("Asset", function()
    ...
    do
        local Constant = "SENSOKSCRIPT.png"
        
        if not isfile(Constant) then
            Asset:Download("https://raw.githubusercontent.com/chhinhlongdev/SenSokScript/main/Assests/Banner.png", Constant) 
        end
    end
    ...
    function Plugins:Community()
        local Community = Plugins:NewPage(115960025411300) do
            local _1 = Plugins:Section(Community, { "Community", Color3.fromRGB(85, 255, 127) }) do
                local Banner = Asset:Get("SENSOKSCRIPT.png") do
